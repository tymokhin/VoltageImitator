...
static void MX_HRTIM1_Init(void)
{
  ...
  pBurstModeCfg.ClockSource = HRTIM_BURSTMODECLOCKSOURCE_FHRTIM;
  ...
  if (HAL_HRTIM_BurstDMAConfig(&hhrtim1, HRTIM_TIMERINDEX_TIMER_A, HRTIM_BURSTDMA_CMP1|HRTIM_BURSTDMA_CMP2) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  pCompareCfg.CompareValue = PWM_PERIOD/5;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, LCD_DS_RS_Pin|LCD_CS_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, LAN_CFG_Pin|LAN_RESET_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pins : LCD_DS_RS_Pin LCD_CS_Pin LAN_CFG_Pin LAN_RESET_Pin */
  GPIO_InitStruct.Pin = LCD_DS_RS_Pin|LCD_CS_Pin|LAN_CFG_Pin|LAN_RESET_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
